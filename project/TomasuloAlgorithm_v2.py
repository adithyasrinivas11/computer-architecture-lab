...
cols = 2
rows = 16
FPRegister = [[0 for i in range(cols)] for j in range(rows)]		#Column 0: Register (R0 to R16), Col1: Busy State
for i in range(rows):
...
#Exection Modules
def add_d(sno,start_time):		#integer addition#
	
	# if(FPRegister[ReservationStAdd[sno][4]][1]==False and FPRegister[ReservationStAdd[sno][5]][1]== False):#check id operands are busy or not
...
def add_floating(sno,start_time):	#floating point addition#

	# if(FPRegister[ReservationStAdd[sno][4]][1]==False and FPRegister[ReservationStAdd[sno][5]][1]== False):#check id operands are busy or not
	op1=FPRegister[int(ReservationStAdd[sno][4].split('R')[1])][0]
	op2=FPRegister[int(ReservationStAdd[sno][5].split('R')[1])][0]
	if (ReservationStAdd[sno][3]=='FADD'):
		op3=op1+op2
	else:
		op3=op1-op2
...
def mul_d(sno,start_time):	#  multiplication#

	# if(FPRegister[ReservationStMul[sno][4]][1]==False and FPRegister[ReservationStMul[sno][5]][1]== False):#check id operands are busy or not
...
def mul_floating(sno,start_time):	#floating point multiplication#
...
def LogicUnit(sno,start_time):	#Logic operations
	if(ReservationStLogicShift[sno][3] == 'CMP'):
		# if FPRegister[ReservationStLogicShift[sno][4]][1] == False:
		op1=FPRegister[int(ReservationStLogicShift[sno][4].split('R')[1])][0]
		op3 = ~op1
		end_time = start_time + 0

	else:
		# if(FPRegister[ReservationStLogicShift[sno][4]][1] == False and FPRegister[ReservationStLogicShift[sno][5]][1] == False):
		op1=FPRegister[int(ReservationStLogicShift[sno][4].split('R')[1])][0]
		op2=FPRegister[int(ReservationStLogicShift[sno][5].split('R')[1])][0]
		if(ReservationStLogicShift[sno][3] == 'XOR'):
			op3 = op1 ^ op2
...
def ShiftUnit(sno,start_time):	#Shift Operations
	# if(FPRegister[ReservationStLogicShift[sno][4]][1] == False and FPRegister[ReservationStLogicShift[sno][5]][1] == False):
	op1=FPRegister[int(ReservationStLogicShift[sno][4].split('R')[1])][0]
	op2=FPRegister[int(ReservationStLogicShift[sno][5].split('R')[1])][0]
...
def ClockZeroQueue():
	...
	while(1):
		...
		if InstrTemp[0] == 'ADD' or InstrTemp[0] == 'FADD' or InstrTemp[0] == 'MUL' or InstrTemp[0] == 'FMUL' or InstrTemp[0] == 'XOR' or InstrTemp[0] == 'NAND' or InstrTemp[0] == 'LSL' or InstrTemp[0] == 'LSR':
	...
	for i in range(1,4):
		if LoadBuffer[i][7] == 0 and LoadBuffer[i][2] == True:		#Busy but hasn't started (means ready to start after issue)
			if LoadBuffer[i][5] == 0: 		#No Tag, then it has to be executed else has to wait further
				...
				InstrStatus[LoadBuffer[i][9]][2] = clock
		...
		if ReservationStAdd[i][9] == 0 and ReservationStAdd[i][2] == True:
			if ReservationStAdd[i][7] == 0 and ReservationStAdd[i][8] == 0:
				...
				if ReservationStAdd[i][3] == 'ADD' or ReservationStAdd[i][3] == 'SUB':
					Result, ReservationStAdd[i][10] = add_d(i, clock)
				elif ReservationStAdd[i][3] == 'FADD' or ReservationStAdd[i][3] == 'FSUB':
					Result, ReservationStAdd[i][10] = add_floating(i, clock)
		...
		if i!=3:
			if ReservationStMul[i][9] == 0 and ReservationStMul[i][2] == True:
				if ReservationStMul[i][7] == 0 and ReservationStMul[i][8] == 0:
					...
					if ReservationStMul[i][3] == 'MUL':
						Result, ReservationStMul[i][10] = mul_d(i, clock)
					elif ReservationStMul[i][3] == 'FMUL':
						Result, ReservationStMul[i][10] = mul_floating(i, clock)
			...
			if ReservationStLogicShift[i][9] == 0 and ReservationStLogicShift[i][2] == True:
				if ReservationStLogicShift[i][7] == 0 and ReservationStLogicShift[i][8] == 0:
					...
					if ReservationStLogicShift[i][3] == 'XOR' or ReservationStLogicShift[i][3] == 'NAND' or ReservationStLogicShift[i][3] == 'CMP':
						...
						Result, ReservationStLogicShift[i][10] = ShiftUnit(i, clock)
...
def CompleteExec(clock):
	for i in range(1,4):
		if LoadBuffer[i][8] == clock:
			InstrStatus[LoadBuffer[i][9]][3] = clock
			LoadBuffer[i][0] = clock + 1
	...
	for i in range(1,4):
		if LoadBuffer[i][0] == clock:
			if cdb == False:
				...
				RS_Name = RegisterStatus[int(LoadBuffer[i][4].split('R')[1])][1]
				RegisterStatus[int(LoadBuffer[i][4].split('R')[1])][1] = 0
				LoadBuffer[i][4] = 0
...
def MainFunction():
	...
	while(1):			
		...
		if(front <= rear):
			tempInstr = InstrQueue[front].split()
			# print(tempInstr)
	
			if tempInstr[0] == 'LDR':
				for i in range(1,4):
					if(LoadBuffer[i][2] == False):

						InstrStatus[front+1][1] = clock 	#set Issue

						LoadBuffer[i][2] = True		#Now it is busy
						...
						CheckWAW(i,'Load')
						RegisterStatus[int(LoadBuffer[i][4].split('R')[1])][1] = LoadBuffer[i][1]
			...
			elif tempInstr[0] == 'CMP' or tempInstr[0] == 'XOR' or tempInstr[0] == 'NAND' or tempInstr[0] == 'LSL' or tempInstr[1] == 'LSR':
				for i in range(1,4):
